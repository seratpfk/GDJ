package com.gdu.app07.repogitory;
...
public class BoardDAO {
	...
	public List<BoardDTO> selectAllBoards() {
      ...
      try {
    	  con = dataSource.getConnection(); // getConnection private(빨간색)
	...
	public int insertBoard(BoardDTO board) {
		...
		try {
			...
			sql = "INSERT INTO BOARD(BOARD_NO, TITLE, CONTENT, WRITER, CREATE_DATE, MODIFY_DATE)"
					+ " VALUES(BOARD_SEQ.NEXTVAL, ?, ?, ?, TO_CHAR(SYSDATE, 'YYYY-MM-DD'), TO_CHAR(SYSDATE, 'YYYY-MM-DD'))"; // 텍스트 자를 때 맨 앞 띄어쓰기
			ps = con.prepareStatement(sql);
			ps.setString(1, board.getTitle()); // 첫번째 물음표에 들어갈 값
