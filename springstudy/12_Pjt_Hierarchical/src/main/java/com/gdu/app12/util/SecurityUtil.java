...
public class SecurityUtil {
	...
	public String sha256(String str) {
		...
		for(int i = 0; i < b.length; i++) {
			sb.append(String.format("%2X", b[i])); // %2X (2자리 16진수(0~F)), 16진수 2글자로 바꾼다.
		}
		return sb.toString();
	}
	...
	public String getAuthString(int length) {
		...
		for(int cnt = 0; cnt < length; cnt++) { // length만큼 반복
			if(secureRandom.nextDouble() < 0.5) { // 50% 확률로
				sb.append( (int)(secureRandom.nextInt(10) + '0') );
			} else {
				sb.append( (int)(secureRandom.nextInt(26) + 'A') );
			}
		}
