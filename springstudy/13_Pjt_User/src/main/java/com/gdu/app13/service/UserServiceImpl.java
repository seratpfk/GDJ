...
public class UserServiceImpl implements UserService {
	...
	public void join(HttpServletRequest request, HttpServletResponse response) {
		// 파라미터
		String id = request.getParameter("id"); // join.jsp의 id
		...
		name = securityUtil.preventXSS(name); // 태그공격(알럿창의 작은따옴표 등) 다 막아줌
		String birthday = birthmonth + birthdate;
		detailAddress = securityUtil.preventXSS(detailAddress);
		int agreeCode = 0; // 필수 동의
		if(!location.isEmpty() && promotion.isEmpty()) { // location은 jsp의 약관 동의 여부의 name속성이기 때문에 null이 될 수 없음(location != null && promotion == null 안됨)
			agreeCode = 1; // 필수 + 위치
	...
	public void retire(HttpServletRequest request, HttpServletResponse response) {
		...
		// 탈퇴할 회원의 userNo, id, joinDate는 session의 loginUser에 저장되어있다.
...
try {
	...
	public void login(HttpServletRequest request, HttpServletResponse response) {
		...
		// id, pw가 일치하는 회원이 있다! : 로그인 기록 남기기 + session 에 loginUser 저장하기
		if(loginUser != null) {
			...
			// 로그링 처리를 위해서 session에 로그인 된 사용자 정보를 올려둠
			request.getSession().setAttribute("loginUser", userMapper.selectUserById(id));
		...
		// id, pw가 일치하는 회원이 없다 : 로그인 페이지로 돌려 보내기
			...
			try {
				...
				PrintWriter out = response.getWriter();
					...
					out.println("location.href='" + request.getContextPath()  + "';"); // 로그인 실패 시 웰컴페이지로 이동
					out.println("</script>");
