...
CREATE TABLE STUDENT(
    ...
    STU_NAME VARCHAR2(20 BYTE) NULL,
    PRIMARY KEY(STU_NO),
    FOREIGN KEY(SCH_CODE) REFERENCES SCHOOL(SCH_CODE) -- 외래키 : SCH_CODE, 참조 : SCHOOL(SCH_CODE)  
);
DROP TABLE STUDENT;
CREATE TABLE STUDENT(
    STU_NO CHAR(5 BYTE) NOT NULL,
    ...
    CONSTRAINT FK_STUDENT_SCHOOL FOREIGN KEY(SCH_CODE) REFERENCES SCHOOL(SCH_CODE) -- STUDENT테이블과 SCHOOL테이블을 연결하는 CONSTRAINT(이름) 
);
