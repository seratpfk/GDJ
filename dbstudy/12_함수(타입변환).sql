...
SELECT 1 + '1'  -- SELECT 1 + TO_NUMBER('1') = 2
  FROM DUAL;

-- '문자'와 '문자' 연산도 모두 숫자로 바꿔서 처리
SELECT '1' + '1' -- SELECT TO_NUMBER('1') + TO_NUMBER('1') = 2
  FROM DUAL;
...
SELECT
     ...
     , TO_CHAR(12345, '9,999') -- ###### (4자리로 지정하였으나 5자리이기 때문에 표시할 수 없음)
     ...
     , TO_CHAR(0.129, '0.00') -- '0.13', 소수 이하 3자리 표기(반올림)
  FROM
      DUAL;
