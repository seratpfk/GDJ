...
SELECT A.ENP_NO, A.NAME, A.POSITION -- 인라인뷰가 조회한 칼럼만 작성할 수 있다.
...
SELECT ROWID, EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWID = 'AAAFBMAABAAALDBAAC';
...
-- 1) WHERE절의 서브쿼리 이용 (1번 문제는 WHERE절이 적합)
...
SELECT B.ROW_NUM, B.EMP_NO, B.NAME, B.SALARY
 ...
 WHERE B.ROW_NUM = 2;  -- 또는 WHERE B.ROW_NUM BETWEEN 2 AND 2;
...
SELECT A.EMP_NO, A.NAME, A.SALARY
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY SALARY DESC) AS ROW_NUM, EMP_NO, NAME, SALARY
          FROM EMPLOYEE) A
 WHERE A.ORW_NUM BETWEEN 2 AND 2;
...
ALTER TABLE EMPLOYEE2
    ADD CONSTRAINT PK_EMPLYEE2 PRIMARY KEY(EMP_NO);
