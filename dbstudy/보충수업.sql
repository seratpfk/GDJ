...
-- 메인쿼리 : SELECT 칼럼 FROM 테이블 WHERE (JOB_ID, DEPARTMENT_ID) = (서브쿼리)  : 단일행
-- 메인쿼리 : SELECT 칼럼 FROM 테이블 WHERE (JOB_ID, DEPARTMENT_ID) IN (서브쿼리) : 다중행
...
-- 가장 많은 사원이 근무 중인 부서 : 부서별 사원수를 구해야 알 수 잇음 -> GROUP BY
