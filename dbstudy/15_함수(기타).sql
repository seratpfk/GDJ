...
--    먼저 입사한 사원이 1등
SELECT
        RANK() OVER(ORDER BY HIRE_DATE ASC) AS 입사순
...
SELECT
        RANK() OVER(PARTITION BY DEPARTMENT_ID ORDER BY SALARY DESC) AS 부서내연봉수
      , EMPLOYEE_ID
      , FIRST_NAME
      , LAST_NAME
      , SALARY
...
SELECT
        EMPLOYEE_ID
      , PHONE_NUMBER
      , DECODE(SUBSTR(PHONE_NUMBER, 1, 3) -- 첫번째 글자부터 3글자만 가져오기
        , '011', 'MOBILE'
        , '515', 'EAST'
        , '590', 'WEST'
        , '603', 'SOUTH'
