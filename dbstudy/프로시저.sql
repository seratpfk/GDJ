...
-- 프로시저 작성 
-- 프로시저 = 쿼리 여러 개일 때 사용
-- 학과번호(DEPNO) 전달하면 소속 학생, 교수, 학과 모두 제거하는 프로시저 만들기
...
IS -- 또는 AS / 변수선언
BEGIN -- 시작
    DELETE FROM STUDENT WHERE DEPTNO = DNO; -- 학생 제거
    DELETE FROM PROFESSOR WHERE DEPTNO = DNO; -- 교수 제거
    DELETE FROM DEPARTMENT WHERE DEPTNO = DNO; -- 학과 제거
    COMMIT;
EXCEPTION -- 예외처리
    WHEN OTHERS THEN
